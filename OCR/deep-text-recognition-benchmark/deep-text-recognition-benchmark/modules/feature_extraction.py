...
class VGG_FeatureExtractor(nn.Module):
    ...
    def __init__(self, input_channel, output_channel=512):
        ...
        self.ConvNet = nn.Sequential(
            ...
            nn.Conv2d(self.output_channel[1], self.output_channel[2], 3, 1, 1), nn.ReLU(True), # 265x8x25
            nn.Conv2d(self.output_channel[2], self.output_channel[2], 3, 1, 1), nn.ReLU(True),
            nn.MaxPool2d((2, 1), (2, 1)), # 265x4x25
...
class RCNN_FeatureExtractor(nn.Module):
    ...
    def __init__(self, input_channel, output_channel=512):
        ...
        self.ConvNet = nn.Sequential(
            nn.Conv2d(input_channel, self.output_channel[0], 3, 3, 1), nn.ReLU(True),
...
class GRCLInit(nn.Module):
    def __init__(self, output_channel):
        super(GRCLInit, self).__init__()
...
class ResNet(nn.Module):
    def __init__(self, input_channel, output_channel, block, layers):
        ...
        self.conv2 = nn.Conv2d(self.output_channel_block[1], self.output_channel_block[1], 
                                kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(self.output_channel_block[2])
